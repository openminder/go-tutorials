...
func filenamesFromCommandLine() (inFilename, outFilename string, err error) {
	...
	if inFilename != "" && inFilename == outFilename {
		log.Fatal("won't override the input file")
	}
...
func americanise(inFile io.Reader, outFile io.Writer) (err error) {
	...
	// after converting to reader and writer the content can accessed as bytes
	reader := bufio.NewReader(inFile)
	writer := bufio.NewWriter(outFile)
	// create anonymous defer function to flush the writers buffer before return control to its caller
	defer func() {
		if err == nil {
			err = writer.Flush()
		}
